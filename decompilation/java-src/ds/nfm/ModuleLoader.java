...
{
    public static Module loadMod(final String pathname) throws FileNotFoundException, IOException {
        return loadMod(new ZipInputStream(new FileInputStream(new File(pathname))));
    }
    ...
    public static Module loadMod(final byte[] buf) throws IOException {
        return loadMod(new ZipInputStream(new ByteArrayInputStream(buf)));
    }
    ...
    public static Module loadMod(final ZipInputStream zipInputStream) throws IOException {
        ...
        if (i == -1) {
            ...
            int off = 0;
            while (i > 0) {
                final int read2 = zipInputStream.read(byteArray, off, i);
                off += read2;
                i -= read2;
            }
        }
